  ...
  Eigen::RowVector3d & t)
{
  ...
  //
  //                 [ X₁ - P₁ ]
  // min ∑( nᵀAu + nᵀ[ X₂ - P₂ ] )²    ,   uϵℝ⁶ => (α,β,ɣ, tϵℝ³)
  //                 [ X₃ = P₃ ]
  //
  //                 [X_i - P_i]
  //
  //                                            [ X₁ - P₁ ]
  // min ( [ diag(N₁) diag(N₂) diag(N₃) ] (Au + [ X₂ - P₂ ]) )²    ,   uϵℝ⁶ => (α,β,ɣ, tϵℝ³)
  //                                            [ X₃ = P₃ ]  
  //
  //           [  0    X₃  -X₂  1  0  0 ]
  // where A = [ -X₃   0    X₁  0  1  0 ]
  ...
  // construct the Nn matrix using the .asDiagonal() turning a vector into a diagonal mat
  Eigen::MatrixXd Nn = Eigen::MatrixXd::Zero(X.rows(), 3*X.rows());
  Nn.block(0,          0, X.rows(), X.rows()) = N.col(0).asDiagonal();
  Nn.block(0,   X.rows(), X.rows(), X.rows()) = N.col(1).asDiagonal();
  Nn.block(0, 2*X.rows(), X.rows(), X.rows()) = N.col(2).asDiagonal();
  ...
  ata = A.transpose()*Nn.transpose()*Nn*A;
  atxipi = -1.0*A.transpose()*Nn.transpose()*Nn*XiPi;
